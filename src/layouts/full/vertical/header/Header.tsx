...
export default function Header() {
  ...
  const getBreadcrumb = () => {
    ...
    if (path.startsWith('/estoque/de-para')) return { icon: deParaIcon, label: 'De-Para de SKU', isImage: true };
    ...
    // Configurações
    if (path.startsWith('/admin/usuarios')) return { icon: '👥', label: 'Usuários' };
    if (path.startsWith('/admin/cargos')) return { icon: '🎭', label: 'Cargos' };
    if (path.startsWith('/admin/convites')) return { icon: '✉️', label: 'Convites' };
    if (path.startsWith('/admin/alertas')) return { icon: '🚨', label: 'Alertas' };
    if (path.startsWith('/admin/seguranca')) return { icon: '🔒', label: 'Segurança' };
    
    if (path.startsWith('/admin/perfil')) return { icon: '👤', label: 'Perfil' };
    if (path.startsWith('/admin')) return { icon: '⚙️', label: 'Geral' };
