/**
 * 📝 MODAL DE ANOTAÇÕES DA VENDA
 * Permite adicionar notas e observações sobre vendas
 */
...
interface VendasAnotacoesModalProps {
  open: boolean;
...
export function VendasAnotacoesModal({
  open,
  onOpenChange,
  orderId,
  packId,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh]">
        <DialogHeader>
          <DialogTitle>Anotações da Venda</DialogTitle>
          <DialogDescription>
            Order ID: <span className="font-mono font-semibold">{orderId}</span>
            {packId && (
              <> • Pack: <span className="font-mono font-semibold">{packId}</span></>
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea
              id="anotacao"
              placeholder="Adicione observações, análises ou informações importantes sobre esta venda..."
              value={anotacao}
