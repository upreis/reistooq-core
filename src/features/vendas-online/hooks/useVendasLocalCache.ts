/**
 * 🚀 COMBO 2.1 - HOOK DE CACHE LOCAL VENDAS CANCELADAS
...
const CACHE_KEY = 'VENDAS_CANCELADAS_LOCAL_CACHE_V1';
...
function restoreCacheSync(): CacheEntry | null {
  try {
    ...
    if (isExpired) {
      console.log('⏰ [VENDAS CACHE] Cache expirado, removendo...');
      localStorage.removeItem(CACHE_KEY);
      return null;
    }
    
    console.log('✅ [VENDAS CACHE] Cache restaurado:', {
    ...
    console.warn('⚠️ [VENDAS CACHE] Erro ao restaurar cache:', e);
    return null;
  }
}

export function useVendasLocalCache() {
  ...
  const saveToCache = useCallback((data: any[], filters: CacheFilters, totalCount: number) => {
    ...
    try {
      ...
      console.log('💾 [VENDAS CACHE] Dados salvos:', {
        items: data.length,
        total: totalCount,
        filtros: filters
      });
    } catch (e) {
      console.warn('⚠️ [VENDAS CACHE] Erro ao salvar cache:', e);
    }
  }, []);
  ...
  const isCacheValidForFilters = useCallback((filters: CacheFilters): boolean => {
    ...
    console.log('🔍 [VENDAS CACHE] Validação:', {
