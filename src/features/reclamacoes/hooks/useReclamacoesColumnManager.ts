/**
 * 🎛️ HOOK PARA GERENCIAMENTO AVANÇADO DE COLUNAS - RECLAMAÇÕES
...
const STORAGE_KEY = 'reclamacoes-column-preferences-v1';
...
const getInitialState = (): ColumnState => {
  ...
  console.log('🎛️ [RECLAMACOES COLUMNS] Estado inicial:', {
...
const loadStoredPreferences = (): Partial<ColumnState> => {
  try {
    ...
    if (parsed.version !== STORAGE_VERSION) {
      console.log('🔄 [RECLAMACOES COLUMNS] Versão de cache desatualizada, limpando...');
    ...
    console.log('💾 [RECLAMACOES COLUMNS] Preferências carregadas:', {
...
const savePreferences = (state: ColumnState) => {
  try {
    ...
    console.log('✅ [RECLAMACOES COLUMNS] Preferências salvas');
  } catch (error) {
...
export const resetReclamacoesColumnCache = () => {
  try {
    localStorage.removeItem(STORAGE_KEY);
    console.log('🔄 [RECLAMACOES COLUMNS] Cache limpo');
    return true;
  } catch (error) {
...
export const useReclamacoesColumnManager = (): UseColumnManagerReturn => {
  ...
  // Salvar automaticamente quando o estado mudar (com debounce implícito via setState)
  useEffect(() => {
