...
export const Devolucao2025Table = ({ accounts, devolucoes, isLoading, error, visibleColumns }: Devolucao2025TableProps) => {
  ...
  // Efeito para medir larguras e a posição do topo
  useEffect(() => {
    if (headerRef.current) {
      ...
      // Calcula a posição do topo baseada no elemento sentinela
      const topPosition = sentinelRef.current?.getBoundingClientRect().top || 0;
  ...
  // Função para renderizar o conteúdo do cabeçalho
    ...
    <div className="w-full">
      <div ref={tableContainerRef} className="border rounded-md overflow-x-auto overflow-y-visible">
        <Table className="min-w-max" style={{ tableLayout: 'fixed' }}>
          ...
          {/* Cabeçalho Fantasma (ocupa espaço para evitar o pulo) */}
          {isSticky && (
