    ...
    <div className="md:col-span-2 grid grid-cols-2 gap-2">
      {isLoading ? (
        ...
        Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="flex flex-col items-center justify-center gap-2 p-3 bg-background border border-border rounded-2xl animate-pulse"
          >
        ...
        Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="flex flex-col items-center justify-center gap-2 p-3 bg-background border border-border rounded-2xl"
          >
        ...
        topProducts.map((product, index) => (
          <div
            key={product.item_id}
            className="flex flex-col items-center justify-center gap-2 p-2 bg-background border border-border rounded-2xl hover:bg-accent/30 transition-all group overflow-hidden"
          >
            {/* Imagem do produto */}
            <div className="relative w-16 h-16 rounded-xl overflow-hidden bg-muted flex-shrink-0">
              ...
              {/* Badge de ranking */}
              <div className="absolute top-0 left-0 bg-primary text-primary-foreground text-[8px] font-bold px-1.5 py-0.5 rounded-br-lg">
                #{index + 1}
              </div>
              {/* Badge de vendas */}
              <div className="absolute bottom-0 right-0 bg-green-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-tl-lg">
                {product.vendas}x
              </div>
            </div>
            
            {/* Nome do produto */}
            <span className="text-[9px] font-medium text-foreground text-center leading-tight line-clamp-2 w-full px-1">
              {truncateTitle(product.item_title, 50)}
            </span>
          </div>
        ))
      )}
