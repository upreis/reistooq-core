    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {isLoading ? (
        ...
        Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="flex flex-col items-center justify-center gap-3 p-4 bg-background border border-border rounded-2xl animate-pulse h-[180px]"
          >
        ...
        Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="flex flex-col items-center justify-center gap-3 p-4 bg-background border border-border rounded-2xl h-[180px]"
          >
        ...
        topProducts.map((product, index) => (
          <div
            key={product.item_id}
            className="flex flex-col items-center justify-center gap-3 p-4 bg-background border border-border rounded-2xl hover:border-primary/50 transition-all group h-[180px]"
          >
            {/* Imagem do produto */}
            <div className="relative w-20 h-20 rounded-xl overflow-hidden bg-muted flex-shrink-0">
              ...
              {/* Badge de vendas */}
              <div className="absolute bottom-0 right-0 bg-green-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-tl-lg">
                {product.vendas}x
              </div>
            </div>
            
            {/* Nome do produto */}
            <span className="text-xs font-medium text-foreground text-center leading-tight line-clamp-2 w-full px-1">
              {truncateTitle(product.item_title, 45)}
            </span>
          </div>
        ))
      )}
