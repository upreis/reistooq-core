...
export default function SimplePedidosPageRefactored({ className }: Props) {
  ...
  const handleSelectionChange = useCallback((selectedRows: Row[]) => {
    const selectedIds = new Set(selectedRows.map(row => row.id || row.unified?.id || ''));
  ...
  return (
    <div className={className}>
      ...
      {pedidosData.showBaixaModal && (
        <BaixaEstoqueModal
          isOpen={pedidosData.showBaixaModal}
          onClose={() => pedidosData.setShowBaixaModal(false)}
          selectedOrders={Array.from(pedidosData.selectedOrders)}
          onSuccess={() => {
            pedidosData.setShowBaixaModal(false);
            pedidosData.clearSelection();
            handleRefresh();
          }}
        />
