...
const ContainerVisualization: React.FC<ContainerVisualizationProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Faixa principal sempre visível */}
      {containersNeeded > 1 && (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200 transition-colors text-sm font-medium"
              >
          ...
          {/* Container principal (último não cheio) sempre visível */}
          {(() => {
            const lastIncompleteContainer = containers.find(c => !c.isFull);
            if (!lastIncompleteContainer) return null;
            
            const status = getContainerStatus(lastIncompleteContainer);
            return (
              <div className="mt-4">
                <div className="flex items-center space-x-4 p-3 bg-white dark:bg-slate-800 rounded-lg border">
                  ...
                  <div className="flex-1 grid grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <div className="text-slate-600 dark:text-slate-400">
                        {lastIncompleteContainer.containerVolume.toFixed(1)}m³ ({lastIncompleteContainer.volumePercent.toFixed(1)}%)
                      </div>
                    ...
                    <div>
                      ...
                      <div className="text-slate-600 dark:text-slate-400">
                        {lastIncompleteContainer.containerWeight.toFixed(0)}kg ({lastIncompleteContainer.weightPercent.toFixed(1)}%)
                      </div>
                      <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1.5 mt-1">
                        <div 
                          className="h-1.5 rounded-full transition-all duration-500"
          ...
          {isExpanded && (
            <div className="mt-4 animate-fade-in">
              {/* Todos os containers não cheios */}
              <div className="space-y-3">
                {containers
                  .filter(container => !container.isFull)
                  .map((container, index) => {
