...
function getMlConfig() {
  ...
  console.log('[OAuth][start] ML credentials configured')
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Client autenticado (para RPC que usa auth.uid())
    const authClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_ANON_KEY') ?? '', {
      global: { headers: { Authorization: authHeader } },
    })

    // -----------------------------
    // ✅ Belt & suspenders: garante a organização (idempotente)
    try {
    ...
    // Busca/garante organização para persistir no oauth_states (fallback se ainda vier null)
    ...
    if (!orgId) {
      ...
      return new Response(JSON.stringify({ success: false, error: 'User organization not found' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
