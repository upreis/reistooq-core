-- Remover todas as versões da função duplicada
DROP FUNCTION IF EXISTS public.get_historico_vendas_safe;

-- Recriar a função correta com todos os parâmetros necessários
...
BEGIN
  ...
  IF _integration_account_id IS NULL THEN
    -- Buscar todas as contas integradas do usuário através da organization
    RETURN QUERY
    SELECT hv.*
    FROM historico_vendas hv
    INNER JOIN integration_accounts ia ON hv.integration_account_id = ia.id
    INNER JOIN profiles p ON p.organizacao_id = ia.organization_id
    WHERE p.id = auth.uid()
  ...
  ELSE
    ...
    IF NOT EXISTS (
      ...
      INNER JOIN profiles p ON p.organizacao_id = ia.organization_id
      WHERE ia.id = _integration_account_id AND p.id = auth.uid()
    ) THEN
