-- =====================================================
-- SECURITY FIX: Parte 2 - Funções restantes (corrigido)
-- =====================================================
...
BEGIN
  ...
  INTO contador FROM public.pedidos_compra 
...
BEGIN
  ...
  SELECT NOT EXISTS (
    ...
    WHERE (user_id = p_user_id OR role_id = p_role_id)
      ...
      AND NOT (start_time <= v_current_time AND v_current_time <= end_time)
  ) INTO has_access;
...
COMMENT ON FUNCTION public.gerar_numero_pedido_compra() IS 'Gera número PC. SET search_path = public.';
...
COMMENT ON FUNCTION public.count_baixados() IS 'Conta baixados. SET search_path = public.';