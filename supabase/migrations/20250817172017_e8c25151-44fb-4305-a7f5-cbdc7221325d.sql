...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='tiny_v3_tokens' AND policyname='tiny3_tokens_deny_all'
  ) THEN
    EXECUTE 'CREATE POLICY tiny3_tokens_deny_all ON public.tiny_v3_tokens
             FOR ALL TO authenticated USING (false) WITH CHECK (false)';
  END IF;
END $$;
