-- Adicionar campos ML faltantes na tabela historico_vendas
ALTER TABLE public.historico_vendas 
  ADD COLUMN IF NOT EXISTS custo_fixo_meli numeric DEFAULT 0,
  ADD COLUMN IF NOT EXISTS conditions text,
  ADD COLUMN IF NOT EXISTS power_seller_status text,
  ADD COLUMN IF NOT EXISTS level_id text,
  ADD COLUMN IF NOT EXISTS marketplace text DEFAULT 'Mercado Livre',
  ADD COLUMN IF NOT EXISTS status_resumos text;

-- Recriar função get_historico_vendas_browse com TODOS os campos
...
RETURNS TABLE (
  ...
  custo_envio_seller numeric, receita_flex_bonus numeric, custo_fixo_meli numeric,
  ...
  endereco_cidade text, endereco_uf text, endereco_bairro text, endereco_rua text, 
  endereco_cep text, endereco_numero text,
  ...
  pack_status text, pack_status_detail text, status_resumos text,
  
  -- Datas
  data_pedido date, data_prevista date, date_created timestamptz,
  last_updated timestamptz, ultima_atualizacao timestamptz, data_criacao_ml timestamptz,
  ...
  -- ML Seller Info
  conditions text, condicao text, power_seller_status text, medalha text,
  level_id text, reputacao text, marketplace text,
  ...
  -- Tags e metadata
  tags text[], tags_pedido text, meta jsonb,
...
AS $$
  ...
  SELECT 
    ...
    hv.custo_envio_seller, hv.receita_flex_bonus, hv.custo_fixo_meli,
    ...
    END AS cpf_cnpj,
    -- Endereço com aliases
    hv.cidade, hv.uf, hv.bairro, hv.rua, hv.cep,
    hv.cidade AS endereco_cidade,
    hv.uf AS endereco_uf,
    hv.bairro AS endereco_bairro,
    hv.rua AS endereco_rua,
    hv.cep AS endereco_cep,
    hv.numero AS endereco_numero,
    -- Status
    ...
    hv.pack_status, hv.pack_status_detail, hv.status_resumos,
    -- Datas
    hv.data_pedido, hv.data_prevista, hv.date_created,
    hv.last_updated, hv.ultima_atualizacao,
    hv.date_created AS data_criacao_ml,
    ...
    -- ML Seller Info com aliases
    hv.conditions, hv.conditions AS condicao,
    hv.power_seller_status, hv.power_seller_status AS medalha,
    hv.level_id, hv.level_id AS reputacao,
    hv.marketplace,
    -- Origem
    ...
    -- Tags
    hv.tags,
    ARRAY_TO_STRING(hv.tags, ', ') AS tags_pedido,
    hv.meta,
    hv.created_by
...
COMMENT ON FUNCTION public.get_historico_vendas_browse IS 'Função completa para listagem de histórico com TODOS os campos ML incluindo custo_fixo_meli, conditions, power_seller_status, level_id, marketplace, status_resumos';
