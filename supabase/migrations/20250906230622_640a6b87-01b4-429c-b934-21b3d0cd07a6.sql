-- Fix failed step by dropping and recreating safe views; finalize policies

-- 1) Drop existing safe views to allow column layout changes
DROP VIEW IF EXISTS public.clientes_safe CASCADE;
DROP VIEW IF EXISTS public.historico_vendas_safe CASCADE;

-- 2) Recreate clientes_safe with masking and org filter
...
-- 3) Ensure historico policy and recreate historico_vendas_safe
DO $$
BEGIN
  IF EXISTS (
...
CREATE POLICY IF NOT EXISTS "hv_select_org_with_pii_perm"
