...
BEGIN
  ...
  PERFORM 1;
  FOR _r IN (SELECT policyname FROM pg_policies WHERE schemaname='public' AND tablename='historico_vendas') LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.historico_vendas', _r.policyname);
  END LOOP;
...
BEGIN
  ...
  LOOP
    EXECUTE format('ALTER FUNCTION %s SET search_path = pg_catalog, public;', r.rp);
  END LOOP;
END $$;
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      RAISE NOTICE 'Nao foi possivel mover a extensao: %', e.extname;
    END;
  END LOOP;
END $$;