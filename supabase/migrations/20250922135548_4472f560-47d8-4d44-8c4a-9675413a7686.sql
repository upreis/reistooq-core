-- ===================================================================
-- SECURE CLIENTES_SECURE VIEW: Add RLS Protection
-- ===================================================================
-- This migration adds RLS policies to the clientes_secure view to prevent
-- unauthorized access to customer data

-- Enable RLS on the clientes_secure view (if it's a table) or ensure proper access controls
-- Since clientes_secure is a view, we need to ensure it has proper security

-- First, let's check if clientes_secure is a table or view and handle accordingly
DO $$
DECLARE
    object_type text;
BEGIN
    -- Check if clientes_secure exists as a table
    SELECT 'table' INTO object_type
    FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'clientes_secure';
    
    IF object_type = 'table' THEN
        -- If it's a table, enable RLS and add policies
        ALTER TABLE public.clientes_secure ENABLE ROW LEVEL SECURITY;
        
        -- Policy for organization-based access with permission checking
        CREATE POLICY "clientes_secure_org_access" ON public.clientes_secure
          FOR SELECT 
          USING (
        ...
        CREATE POLICY "clientes_secure_admin_manage" ON public.clientes_secure
          FOR ALL 
          USING (
            organization_id = get_current_org_id() 
        ...
        -- Log access to secure customer data
        CREATE OR REPLACE FUNCTION public.log_clientes_secure_access()
        RETURNS trigger
        LANGUAGE plpgsql
        SECURITY DEFINER
        ...
        -- Add trigger for access logging
        DROP TRIGGER IF EXISTS clientes_secure_access_log ON public.clientes_secure;
        CREATE TRIGGER clientes_secure_access_log
          AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.clientes_secure
          FOR EACH ROW EXECUTE FUNCTION log_clientes_secure_access();
          
    ELSE
        -- If it's a view, ensure it has proper security invoker setting
        -- and create a security function to control access
        ...
        -- Update the view to include security check
...
INSERT INTO public.audit_logs (
  ...
  'view', 
  'clientes_secure',
  jsonb_build_object('action', 'added_rls_protection', 'timestamp', now())
);