...
SELECT 
  ...
  SUM(valor_retido) as valor_total,
  AVG(valor_retido) as valor_medio,
  ...
  COUNT(*) FILTER (WHERE dados_order IS NOT NULL) as com_order,
  COUNT(*) FILTER (WHERE dados_fulfillment IS NOT NULL) as com_fulfillment,
  now() as updated_at
FROM public.devolucoes_avancadas
GROUP BY integration_account_id, DATE(data_criacao);
...
CREATE INDEX IF NOT EXISTS idx_dev_order_data 
  ON public.devolucoes_avancadas USING GIN (dados_order)
  WHERE dados_order IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_dev_review_data 
  ON public.devolucoes_avancadas USING GIN (dados_review)
  WHERE dados_review IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_dev_comunicacao_data 
  ON public.devolucoes_avancadas USING GIN (dados_comunicacao)
...
CREATE INDEX IF NOT EXISTS idx_dev_account_status_date 
  ON public.devolucoes_avancadas (integration_account_id, status_devolucao, data_criacao DESC);
...
COMMENT ON MATERIALIZED VIEW public.devolucoes_metrics_cache IS 'Cache de métricas agregadas para dashboard (refresh manual ou via cron job)';
COMMENT ON FUNCTION public.enqueue_background_job IS 'Adiciona um novo job na fila de processamento assíncrono';
COMMENT ON FUNCTION public.get_next_background_job IS 'Retorna e marca como processing o próximo job pendente (com lock SKIP LOCKED para evitar concorrência)';
...
COMMENT ON FUNCTION public.refresh_devolucoes_metrics IS 'Atualiza cache de métricas agregadas (deve ser chamado via cron job ou manualmente)';
