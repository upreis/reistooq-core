-- Desabilitar triggers temporariamente e criar Super Admin
DO $$
DECLARE
  org_id uuid;
...
BEGIN
  ...
  -- Buscar o primeiro usuário da organização
  ...
  -- Se ainda não tem usuário, criar um aviso mas continuar
  IF first_user_id IS NULL THEN
    RAISE NOTICE 'ATENÇÃO: Nenhum usuário encontrado! Super Admin criado mas não atribuído.';
  END IF;
  ...
  -- Se existe usuário, atribuir o cargo
  IF first_user_id IS NOT NULL THEN
  ...
  -- Log da operação
  RAISE NOTICE 'Super Admin criado com sucesso! Organização: %, Usuário: %, Role: %', org_id, first_user_id, super_admin_role_id;
END $$;