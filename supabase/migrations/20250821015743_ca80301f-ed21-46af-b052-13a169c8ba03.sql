-- P0: Normalizar integration_secrets sem downtime
...
-- 3. Migrar dados da tabela backup para nova estrutura
-- Tentar converter account_id::uuid para integration_account_id quando possível
...
SELECT 
  ...
  COALESCE(backup.enc_data, backup.secret_enc) as secret_enc, -- Usar enc_data se secret_enc não existir
  backup.expires_at,
...
WHERE backup.enc_data IS NOT NULL OR backup.secret_enc IS NOT NULL; -- Só migrar se tiver dados criptografados
...
-- 6. Atualizar função encrypt_integration_secret para novo schema
