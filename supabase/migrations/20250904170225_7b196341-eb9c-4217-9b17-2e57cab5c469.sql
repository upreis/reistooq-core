...
DECLARE
  ...
  brinquedos_id uuid;
BEGIN
  -- Buscar o ID da categoria "Bebês" e a organização
  SELECT id INTO bebes_id FROM categorias_produtos WHERE nome = 'Bebês' AND nivel = 1 LIMIT 1;
  SELECT organization_id INTO org_id FROM categorias_produtos WHERE id = bebes_id;
  ...
  RETURNING id INTO roupas_id;
  
  -- Buscar os IDs das categorias recém criadas
  SELECT id INTO roupas_id FROM categorias_produtos WHERE nome = 'Roupas e Acessórios' AND categoria_principal_id = bebes_id;
  SELECT id INTO alimentacao_id FROM categorias_produtos WHERE nome = 'Alimentação' AND categoria_principal_id = bebes_id;
  SELECT id INTO higiene_id FROM categorias_produtos WHERE nome = 'Higiene e Cuidados' AND categoria_principal_id = bebes_id;
  SELECT id INTO brinquedos_id FROM categorias_produtos WHERE nome = 'Brinquedos' AND categoria_principal_id = bebes_id;
