-- Fix RPC overload causing PGRST203 ambiguity error
-- Drop the duplicate signature that is causing conflicts

DROP FUNCTION IF EXISTS public.get_pedidos_masked(date, date, text, integer, integer);

-- Ensure we have only ONE clean signature for get_pedidos_masked
