-- Primeiro, dropar função existente para poder alterá-la
DROP FUNCTION IF EXISTS public.get_historico_vendas_masked(text, date, date, integer, integer);

-- Recriar a função com funcionalidade correta
...
-- Grant admin permission to the specific user for historico:view using overrides
-- Use the known user_id from auth logs
DO $$
DECLARE
...
BEGIN
  ...
  IF v_org IS NOT NULL THEN
    -- Grant the historico:view permission override
    ...
    -- Also ensure all critical admin permissions
    INSERT INTO public.user_permission_overrides (user_id, organization_id, permission_key, allow)
    VALUES 
      (v_user, v_org, 'users:read', true),
      (v_user, v_org, 'roles:manage', true),
      (v_user, v_org, 'invites:manage', true),
      (v_user, v_org, 'system:audit', true),
