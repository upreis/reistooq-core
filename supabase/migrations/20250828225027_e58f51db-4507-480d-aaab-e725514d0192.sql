...
BEGIN
  ...
  -- Criar o cargo Super Admin se não existir (sem description)
  INSERT INTO public.roles (organization_id, name, slug, is_system)
  VALUES (current_org_id, 'Super Admin', 'super_admin', true)
  ...
  -- Remover atribuições antigas de cargo deste usuário
  DELETE FROM public.user_role_assignments WHERE user_id = current_user_id;
  
  -- Atribuir APENAS o cargo Super Admin ao usuário atual
  ...
  -- Log da operação
  RAISE NOTICE 'Super Admin criado com sucesso para usuário % na organização %. Role ID: %', current_user_id, current_org_id, super_admin_role_id;
END $$;