...
BEGIN
  ...
  LOOP
    EXECUTE format('ALTER FUNCTION %s SET search_path = pg_catalog, public;', func.rp);
  END LOOP;
END $$;
...
BEGIN
  FOR ext IN
    ...
    WHERE n.nspname='public' AND extname <> 'pg_net' -- pg_net normalmente não pode ser movida
  LOOP
    BEGIN
