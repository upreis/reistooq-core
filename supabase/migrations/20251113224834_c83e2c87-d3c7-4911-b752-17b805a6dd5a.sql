-- =====================================================
-- SECURITY LINTER FIX: Adiciona SET search_path = public
-- Corrige 31 avisos do Supabase Linter
-- =====================================================
...
AS $$
BEGIN
  RETURN QUERY SELECT hv.id, p_organization_id as organization_id, hv.numero_pedido as order_id,
    public.mask_name(hv.cliente_nome) as buyer_name, public.mask_cpf_cnpj(hv.cpf_cnpj) as buyer_cpf,
    public.mask_email(COALESCE(hv.cliente_nome, '')) as buyer_email, public.mask_phone(COALESCE(hv.cliente_nome, '')) as buyer_phone,
    hv.created_at, hv.valor_total as total_amount
  FROM public.historico_vendas hv 
  WHERE hv.integration_account_id IN (SELECT id FROM public.integration_accounts WHERE organization_id = p_organization_id)
...
SELECT id, organization_id,
  public.mask_name(nome_completo) as nome, public.mask_cpf_cnpj(cpf_cnpj) as cpf_cnpj,
  public.mask_email(email) as email, public.mask_phone(telefone) as telefone, created_at
