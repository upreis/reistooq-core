-- Primeiro, vamos remover a função existente e criar uma nova versão
DROP FUNCTION IF EXISTS public.get_historico_vendas_masked(date, date, text, integer, integer);

-- Criar a nova função com os tipos corretos
...
AS $function$
  SELECT 
    ...
    -- Produtos
    hv.sku_produto,
    COALESCE(hv.descricao, hv.titulo_produto, 'Produto') as descricao,
    COALESCE(hv.quantidade, hv.quantidade_total, 1) as quantidade,
    hv.quantidade_total,
    hv.titulo_produto,
    COALESCE(hv.valor_unitario, hv.valor_total / NULLIF(COALESCE(hv.quantidade, hv.quantidade_total, 1), 0), 0) as valor_unitario,
    ...
    -- Mapeamento
    COALESCE(hv.status, hv.status_baixa, 'processado') as status,
