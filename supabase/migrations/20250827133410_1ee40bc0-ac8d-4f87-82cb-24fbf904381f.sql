-- Melhorar função hv_insert para garantir integration_account_id sempre preenchido
...
BEGIN
  ...
  IF v_integration_account_id IS NULL THEN
    ...
    -- Se AINDA não tem conta, criar uma padrão
    IF v_integration_account_id IS NULL THEN
      INSERT INTO public.integration_accounts (name, provider, organization_id, is_active)
      VALUES ('Sistema Padrão (hv_insert)', 'sistema', v_org_id, true)
