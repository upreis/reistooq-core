-- Simple approach: just create safe views and finalize the policies
...
-- Remove public access to raw tables
...
-- Add policies requiring strong permissions for direct table access
CREATE POLICY "clientes_admin_only" ON public.clientes FOR SELECT
USING (organization_id = public.get_current_org_id() AND public.has_permission('customers:view_pii'));

CREATE POLICY "historico_admin_only" ON public.historico_vendas FOR SELECT
USING (
  EXISTS (
