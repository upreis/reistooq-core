-- CORREÇÃO COMPLETA DOS 4 ERROS DE SEGURANÇA
-- Esta migração corrige todos os problemas sem quebrar funcionalidades

-- 1. CORREÇÃO: Customer Personal Information Could Be Stolen
-- Já corrigido anteriormente com a view clientes_safe com mascaramento automático

-- 2. CORREÇÃO: Customer Data Backup Table Exposed  
-- A clientes_safe é uma view segura, não uma tabela backup
-- Vamos garantir que só há acesso via view segura

-- 3. CORREÇÃO: Sales Data and Customer Information Could Be Accessed
-- Implementar RLS adequada para historico_vendas
...
-- 4. CORREÇÃO: API Keys and Tokens Could Be Stolen by Hackers
-- Já implementado - integration_secrets tem RLS bloqueando acesso a usuários normais
...
-- Remover acesso direto à tabela principal para usuários normais
REVOKE ALL ON public.historico_vendas FROM authenticated;
GRANT SELECT ON public.historico_vendas TO service_role;
