-- 🔒 CORREÇÃO DE SEGURANÇA: Resolver 3 erros críticos (CORRIGIDO)
...
-- Remover a view clientes_safe insegura se existir
...
-- Criar view clientes_safe_secure com mascaramento de dados baseado em permissões
CREATE VIEW clientes_safe_secure AS
SELECT 
  c.id,
  -- Mascarar nome: mostrar apenas primeiras 3 letras + *** se não tem permissão
  CASE 
    WHEN public.has_permission('customers:read_sensitive') THEN c.nome_completo
    ELSE LEFT(c.nome_completo, 3) || '***'
  END as nome_completo,
  -- Mascarar CPF/CNPJ completamente se não tem permissão
  CASE 
    ...
    END
  END as cpf_cnpj,
  -- Mascarar email: mostrar apenas domínio se não tem permissão
  CASE 
    WHEN public.has_permission('customers:read_sensitive') THEN c.email
    WHEN c.email IS NOT NULL THEN '***@' || SPLIT_PART(c.email, '@', 2)
    ELSE NULL
  END as email,
  -- Mascarar telefone: apenas últimos 4 dígitos se não tem permissão
  CASE 
